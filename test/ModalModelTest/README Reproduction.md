...
the Julia REPL **in order** and it reproduces exactly what
`paper_ssnn_training.jl` does. (The script wraps steps 8 onward in a `main()`
function; here they are unwrapped so you can run and inspect each stage
interactively.)
...
Every file the example touches is declared up front as a constant, and all of them live next to the script (@__DIR__). They fall into four groups:

Measurement data (Kraken-generated by default, or user-supplied). MEASUREMENT_FILE (range_m, depth_m, amp, split) is the training data — 1,224 points, 51 ranges × 24 depths in the 650–700 m band — with the 70/30 train/validation split baked into the split column. AOI_FILE (range_m, depth_m, amp) is the dense 0.1 m evaluation grid over 550–800 m × 1–24 m, used only to judge extrapolation, never for training. Step 7 runs Kraken to create both files only if they are missing (or the force flag is set); the pipeline reads them the same way regardless of origin, so real measurements in the same format work identically — just keep RREF inside your own measurement band.

Training outputs (created by steps 11–14). HISTORY_FILE logs one row per checkpoint per restart: restart, epoch, train_objective, train_field_mse, val_field_mse, anchor_rmse_ms, ssp_min_ms, ssp_max_ms, grad_norm. PARAMETER_FILE is every trained parameter of the best run, flattened into parameter/value pairs (e.g. A_re_1 … A_re_6, ssp_W1_1 … ssp_W1_12), so a run can be reloaded or inspected without Julia. SSP_PRED_FILE tabulates the learned profile against the reference on the model depth grid (depth_m, c_true_ms, c_pred_ms, error_ms), and AOI_PRED_FILE does the same for the field on a 0.5 m subgrid of the AOI (amp_true, amp_pred, db_true, db_pred, db_error).

Plots (created by steps 13–15). SSP_PLOT_FILE overlays reference vs. learned SSP with the five anchor measurements; AOI_PLOT_FILE shows Kraken ground truth and model prediction side by side as dB heatmaps with the training band marked; HISTORY_PLOT_FILE is the train/validation MSE curve of the best restart.

Finally, FORCE_REGENERATE_GROUND_TRUTH controls whether step 7 reruns Kraken. Leave it true for the very first run so the CSVs are guaranteed to match this exact setup (depth, frequency, seabed, grids); afterwards set it to false — the Kraken sweep over the 0.1 m AOI grid is by far the slowest part of the pipeline, and regenerating also redraws the train/validation split (fresh entropy), which changes results run to run.
...
Run it (slow the first time — this is the Kraken part):
